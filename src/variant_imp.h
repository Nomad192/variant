...
//    }, other);
  ...
  variant& operator=(const variant& other) {
    if(index() == other.index())
      visit_helper::do_visit([&](auto other_val) -> void {
        multi_union_helper_t<First, Rest...>::template operator_set<get_index_by_type<decltype(other_val), First, Rest...>::index>(storage.index, storage.value, other_val);
        storage.index = get_index_by_type<decltype(other_val), First, Rest...>::index;
      }, other);
    else
      visit_helper::do_visit([&](auto other_val) -> void {
        multi_union_helper_t<First, Rest...>::template set<get_index_by_type<decltype(other_val), First, Rest...>::index>(storage.index, storage.value, other_val);
        storage.index = get_index_by_type<decltype(other_val), First, Rest...>::index;
      }, other);
    return *this;
  }
      ...
      //                             std::is_nothrow_move_constructible<First>>)
  {
    if(index() == other.index())
      visit_helper::do_visit([&](auto other_val) -> void {
        multi_union_helper_t<First, Rest...>::template operator_set<get_index_by_type<decltype(other_val), First, Rest...>::index>(storage.index, storage.value, std::move(other_val));
        storage.index = get_index_by_type<decltype(other_val), First, Rest...>::index;
      }, std::move(other));
    else
      visit_helper::do_visit([&](auto other_val) -> void {
        multi_union_helper_t<First, Rest...>::template set<get_index_by_type<decltype(other_val), First, Rest...>::index>(storage.index, storage.value, std::move(other_val));
        storage.index = get_index_by_type<decltype(other_val), First, Rest...>::index;
      }, std::move(other));
